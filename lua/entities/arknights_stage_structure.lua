...
ENT.OperatorInternalAnimID = "bluepoison"
...
if(CLIENT) then
	...
	function ENT:OpTurnSideHorizontal(right)
		if(right) then
			...
			self.Anim_IdealHorizontal = Arknights.ENUM_Left
		end
		self.Logic_ShouldTurnSide = true
	end
	...
	function ENT:OpTurnAnimation()
		...
		if(!self.Var_TurnFinished) then
			self.Anim_SpriteWRenderScale = math.Clamp(self.Anim_SpriteWRenderScale - Arknights.GetFixedValue(0.08), 0, 1)
	...
	local vec001 = Vector(3, 3, 3)
	local bound_rd = 32767
	local bounds = Vector(1, 1, bound_rd)
	local rndmins, rndmaxs = Vector(-24, -24, 0), Vector(24, 24, 0)
	function ENT:Draw(_, manualPaint)
		if(!manualPaint) then return end
		self:OpThink()
		...
		render.SetColorMaterial()
		render.DrawBox(self:GetPos(), angle000, -vec001, vec001, Color(255, 255, 255, 255))
		cam.IgnoreZ(false)
		local vec = self:GetPos()
		self:SetRenderBoundsWS(vec - bounds, vec + bounds)
	end
